...
static void parse_item(cbor_reader_t const *reader, cbor_item_t const *item,
		    cbor_item_t const *parent, void *arg)
{
	...
	if (key) {
		...
		if (parser && parser->run) {
			parser->run(parser, &reader->msg[item->offset],
					item->size, ctx->arg);
		}
	}
}

static size_t iterate_each(cbor_reader_t const *reader,

...
TEST(Helper, ShouldDispatch_WhenUsedWithCborPathInlineDeclFromC_Depth1)
{
	/* {"certificate": "your-cert", "privateKey": "your-private-key"} */
	static const uint8_t msg[] = {
		0xA2, 0x6B, 0x63, 0x65, 0x72, 0x74, 0x69, 0x66, 0x69,
	...
	LONGS_EQUAL(true, helper_inline_decl_dispatch_depth1(msg, sizeof(msg), &counts));
	LONGS_EQUAL(1, counts.certificate);
	LONGS_EQUAL(1, counts.private_key);
	LONGS_EQUAL(0, counts.dev_url);
	LONGS_EQUAL(0, counts.prod_url);
}

TEST(Helper, ShouldDispatch_WhenUsedWithCborPathInlineDeclFromC_Depth2)
{
	/*
	 * {"dev":  {"ba_id": "evse", "url": "wss://a"},
	 *  "prod": {"ba_id": "user", "url": "wss://b"}}
	...
	LONGS_EQUAL(true, helper_inline_decl_dispatch_depth2(msg, sizeof(msg), &counts));
	LONGS_EQUAL(0, counts.certificate);
	LONGS_EQUAL(0, counts.private_key);
	LONGS_EQUAL(1, counts.dev_url);
	LONGS_EQUAL(1, counts.prod_url);
}
